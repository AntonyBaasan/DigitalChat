...
//            console.log("A user want to disconnect. has :" + self.users.length + " user(s)");

//            var foundUser = _.find(self.users, function (item) {
//                console.log("socket.id :" + socket.id + " item.socket.id" + item.socket.id);
...
//                self.io.sockets.emit('userDisconnected', { fromUser: foundUser.user }); //socket.broadcast.emit('userDisconnected', "User '"user.user"' has disconnected");

//                console.log("User '" + foundUser.user + "' disconnected. left :" + self.users.length + " users");
//            }
//        });
    ...
    self.attachListeners = function (user) {
        user.socket.on("disconnect", function (data) { self.onDisconnect(user, data); });
    ...
    self.onLogout = function (user, data) {
        ...
        if (foundUser) {
            user.socket.broadcast.emit("userLogout", { fromUser: foundUser.user });
            self.users.splice(self.users.indexOf(foundUser), 1);
