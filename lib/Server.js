...
var Server = function (io) {
    ...
    self.onDisconnect = function (user, data) {
        ...
        console.log("User '" + user.user + "' disconnectiong.");
    ...
    self.onTalkToPeer = function (user, data) {
        console.log("User '" + user.user + "' called onTalkToPeer. data['toUser']: " + data['toUser']);
        ...
        if (foundUser)
            foundUser.socket.emit("talkToPeer", { fromUser: user.user, toUser: data['toUser'], content: data['content'] });
    };
